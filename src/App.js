...
const pathsWithoutNav = ['/parkinglayout', '/login', '/signup']

const pathsWithoutTopNav = ['/parkinglayout', '/home', '/login', '/signup']
...
const App = withRouter(({ location }) => {
  const isFullScreen =
    location.pathname === '/'
      ? true
      : pathsWithoutNav.some(path => location.pathname.startsWith(path))
  const isTopFullScreen =
    location.pathname === '/'
      ? true
      : pathsWithoutTopNav.some(path => location.pathname.startsWith(path))
  return (
    <div>
      {isFullScreen ? '' : <Navbar />}
